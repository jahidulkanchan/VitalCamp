  ...
  return (
    <>
      <section className="container py-10 px-5 mx-auto">
        ...
        {popularCamp?.map((item,i)=> <CampCard key={i} camp={item}></CampCard>)}
        </div>
        <div onClick={()=> window.scrollTo(0,0)} className="w-fit mx-auto mt-10"><Link className="text-white bg-primary px-5 py-2 rounded hover:bg-secondary" to='/availableCamp'>See All Camps</Link></div>
      </section>
    </>
  );
};
